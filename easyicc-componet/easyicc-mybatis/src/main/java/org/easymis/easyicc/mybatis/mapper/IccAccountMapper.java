...
public interface IccAccountMapper {
	...
	@Insert("insert into icc_account(id,org_id,member_id,staff_id,code,nick,nick_name,login_start_time,login_end_time,filter_time,max_accept_count,max_chat_count,rank,account_type,login_ip,login_mac,head_url,depict)values(#{id},#{orgId},#{memberId},#{staffId},#{code},#{nick},#{nickName},#{loginStartTime},#{loginEndTime},#{filterTime},#{maxAcceptCount},#{maxChatCount},#{rank},#{accountType},#{loginIp},#{loginMac},#{headUrl},#{depict})")
	public boolean save(IccAccount bean);

	@Insert("insert into icc_account(id,org_id,member_id,staff_id,code,nick,nick_name,login_start_time,login_end_time,filter_time,max_accept_count,max_chat_count,rank,account_type,login_ip,login_mac,head_url,depict)values(#{id},#{orgId},#{memberId},#{staffId},#{code},#{nick},#{nickName},#{loginStartTime},#{loginEndTime},#{filterTime},#{maxAcceptCount},#{maxChatCount},#{rank},#{accountType},#{loginIp},#{loginMac},#{headUrl},#{depict})")
	public void saveBatch(List<IccAccount> beans);

	@Update("UPDATE icc_account SET id= #{id},org_id= #{orgId},member_id= #{memberId},staff_id= #{staffId},code= #{code},nick= #{nick},nick_name= #{nickName},login_start_time= #{loginStartTime},login_end_time= #{loginEndTime},filter_time= #{filterTime},max_accept_count= #{maxAcceptCount},max_chat_count= #{maxChatCount},rank= #{rank},account_type= #{accountType},login_ip= #{loginIp},login_mac= #{loginMac},head_url= #{headUrl},depict= #{depict} WHERE id= #{id}")
	...
	@Results(value = { @Result(property = "id", column = "id"), @Result(property = "orgId", column = "org_id"),
			...
			@Result(property = "maxChatCount", column = "max_chat_count"), @Result(property = "rank", column = "rank"),
